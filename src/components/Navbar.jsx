...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center px-10 py-5 backdrop-blur-2xl bg-gray-100'>
       <Link to='/'> <h2 className='text-4xl font-bold'>Portfolio</h2></Link>
        <ul className={`max-md:absolute max-md:top-0 max-md:left-0 flex items-center gap-10 font-medium  md:px-10 md:py-4 md:rounded-3xl shadow-2xl  max-md:flex-col transition-[width] duration-500 max-md:h-screen  justify-center max-md:text-2xl backdrop-blur max-md:bg-black/70 max-md:text-white ${isOpen ? 'max-md:w-95' : 'max-md:w-0'}`}>
            ...
            <Link onClick={() => {scrollTo(0,0), setIsOpen(false)}} to='/'><li>Home</li></Link>
            <Link onClick={() => {scrollTo(0,0), setIsOpen(false)}} to='/projects'><li>Projects</li></Link>
            <Link onClick={() => {scrollTo(0,0), setIsOpen(false)}} to='/skills'><li>Skills</li></Link>
            <Link onClick={() => {scrollTo(0,0), setIsOpen(false)}} to='/contact'><li>Contact</li></Link>
